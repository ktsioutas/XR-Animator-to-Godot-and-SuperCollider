...
##   - Both sources live: complete SC body + VMC head rotation.
##   - Only one source live: copy that source as a complete body.
##
## The older VMC-upper/SC-legs blend remains available through the explicit
## VMC_UPPER_SC_LEGS mode.
##
...
@export_enum(
	"Auto: SC body + VMC head",
	"SuperCollider only",
	"VMC only",
	"SC body + VMC head",
	"VMC upper + SC legs (legacy)")
...
@export_category("Legacy VMC-upper/SC-legs mixing")
## Used only by VMC_UPPER_SC_LEGS mode. When both trackers are live, VMC
## supplies root/hips/upper body and joint positions; SuperCollider supplies
## rotations for the two leg chains.
...
## In AUTO_SELECT (when both are live) and SC_BODY_VMC_HEAD mode, keep the
## head position attached to the SC body and use only the VMC user's rotation.
...
func _process(_delta: float) -> void:
	...
	match operating_mode:
		...
		OperatingMode.VMC_UPPER_SC_LEGS:
			effective_source = _process_vmc_upper_sc_legs_mode(sc_tracker, vmc_tracker)
...
## It is used by AUTO_SELECT and SC_BODY_VMC_HEAD mode.
...
## It is used only by the legacy VMC_UPPER_SC_LEGS mode.
...
func set_operating_mode(new_mode: int) -> void:
	if new_mode < OperatingMode.AUTO_SELECT or new_mode > OperatingMode.VMC_UPPER_SC_LEGS:
...
func _process_auto_select(
	...
	# New default mixed behavior: SuperCollider owns the complete body pose.
	# VMC contributes only head movement relative to its calibrated neutral pose.
	if _tracker_is_live(sc_tracker) and _tracker_is_live(vmc_tracker):
		return _process_sc_body_vmc_head(sc_tracker, vmc_tracker)
...
## Optional compatibility mode for the previous stage of the project.
func _process_vmc_upper_sc_legs_mode(
		sc_tracker: XRBodyTracker,
		vmc_tracker: XRBodyTracker) -> String:
	if (
...
## Legacy mixed case: begin with a complete VMC pose, then overwrite the
